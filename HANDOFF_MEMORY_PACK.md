...
- ⬜ NEXT STEPS (in order):
  ...
  5. ✅ DONE 2026-09-02: **24-topic guideline library + named-source retrieval** — expanded from 14 to 24 topics covering the WHO/CDC/NICE/ICMR list (added: dengue, malaria, TB, HIV, healthy pregnancy, antibiotic stewardship, high cholesterol, depression, hypoglycaemia, heart attack first aid). Every entry now carries a `source_name` (e.g. "WHO / ICMR", "CDC / NICE") which the UI cites in: the auto-match banner, a "Guideline used for this check" card above results, and the side-by-side evidence panel headers. Entries remain SHORT SIMPLIFIED SUMMARIES of long-established public guidance (not verbatim copyrighted PDFs) — reference field says so; swapping in licensed verbatim chunks is the documented upgrade path. RETRIEVAL HARDENED: overlap audit found 12 keyword collisions + 21 wordy pair overlaps; fixed via distinctive keywords per confusable topic + phrase-keyword scoring (multi-word phrases score 4, single words 2). Matcher test suite now 17/17 + 6/6 disambiguation green. End-to-end verified: dengue question with NO source pasted → retrieves "Dengue fever" (WHO / ICMR) → dangerous ibuprofen answer → BLOCKED with plain-language reason "Ibuprofen can cause dangerous bleeding in dengue and should not be used." New demo presets T9 (dengue wrong painkiller), T10 (dengue correct), T11 (warfarin holistic catch). Deploy verified live.
  6. REMAINING: user manual click-through in a real browser (automation can't commit Streamlit text-area edits). Then optional: tiny NLI third checker, Oracle ARM hosting (unlocks MiniCheck + 24/7), eval set with precision/recall, FastAPI endpoint, phase-2 demo RAG bot.
- ❗ DEPLOY LESSONS (2026-09-02): (a) Streamlit Cloud builder choked on heavy requirements (torch pin + minicheck git install caused half-deployed app + stale ImportError) — FIXED by slimming requirements.txt to streamlit+groq only, cross-checker deps documented as optional comments; app runs fine without them (crosscheckers show 'unavailable' in dropdown); (b) value= + key= together on st widgets is the anti-pattern that reverts user edits — use session_state defaults + key= only, presets via st.rerun(); (c) added on-page import diagnostics trap in app.py (shows real traceback instead of Streamlit's redacted error card); (d) HF Spaces free tier 2026 = no cpu-basic for Streamlit — use Streamlit Cloud or Oracle ARM.
