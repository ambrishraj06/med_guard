...
- ⬜ NEXT STEPS (in order):
  ...
  2. ✅ DONE 2026-09-02: **Ask & Check mode shipped** — question-only usage: built-in guideline library (medguard/library.py, 14 public-guideline topics, keyword+overlap matcher, 9/9 offline tests), grounded answer generator (generate_answer in audit.py — generator uses ONLY the guideline, same epistemology as the auditor). Empty guideline box → auto-match; empty answer box → generate + audit. Live at medguardauidt.streamlit.app.
  3. USER should manually click through question-only mode in a real browser (automation can't commit Streamlit text-area edits — synthetic events don't trigger Streamlit's widget protocol; only real typing/blur does). Local verification with real Groq key PASSED: 'child fever' question → auto-matched Fever-in-children → generated paracetamol/ibuprofen answer → SAFE/100%.
  4. OPTIONAL UPGRADE: tiny NLI cross-checker (cross-encoder/nli-deberta-v3-small ~140MB) as third dropdown entry; Oracle Cloud ARM (12-24GB free) to unlock MiniCheck + 24/7; eval set with precision/recall; FastAPI endpoint; phase-2 demo RAG bot.
