...
- ⬜ NEXT STEPS (in order):
  ...
  7. REMAINING / NEXT SESSION (user agreed to work on suggestions next day, in this priority order):
     1. Prompt-injection delimiter defense (30 min) — battery shows the judge already resists, but add explicit <<<ANSWER>>> delimiters + system-prompt line to harden.
     2. Latency: skip holistic check when verdict already BLOCKED (~33% faster dangerous-path audits).
     3. THE EVAL SET (resume gold): hand-labeled ~20 triplets → precision/recall scorecard script. NOTE the hard battery already provides 20 labeled cases to seed this — half the labeling work is done.
     4. Then optional: semantic retrieval (MiniLM) for slang, verbatim guideline chunks, Fast/Thorough toggle, audit caching, FastAPI endpoint.
