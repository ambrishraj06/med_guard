...
- ⬜ NEXT STEPS (in order):
  ...
  6. REMAINING: user manual click-through in a real browser (automation can't commit Streamlit text-area edits). Then optional: semantic embeddings for slang ("my back is killing me" currently abstains — correct but conservative), tiny NLI third checker, Oracle ARM hosting, eval set, FastAPI endpoint.
