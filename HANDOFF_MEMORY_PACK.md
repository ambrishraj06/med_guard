...
- ⬜ NEXT STEPS (in order):
  ...
  14. REMAINING SESSIONS: Session 5 "ENSEMBLE LIVE" (optional — HHEM ONNX-quantized ~250MB on Streamlit Cloud so the per-claim second referee runs in production; note the current Cloud deploy ALREADY loaded full HHEM once when a heavier build slipped through, so it fits occasionally — ONNX would make it reliable).
