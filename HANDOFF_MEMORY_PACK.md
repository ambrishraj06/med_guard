...
- ⬜ NEXT STEPS (in order):
  ...
  9. REMAINING SESSIONS (master plan, see analysis): Session 2 "THE PROOF" (eval set + judge agreement — needs user 20 min labeling), Session 3 "SHARPER" (staged progress, Fast/Thorough toggle, caching, history, keyword cleanup, mobile @media, empty state), Session 4 "STOREFRONT" (README validation section + transformers<5 pin + auto-pick docs, LICENSE MIT, remove HF frontmatter, page title), Session 5 "ENSEMBLE LIVE" (HHEM ONNX on Streamlit Cloud).
