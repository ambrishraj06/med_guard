...
- ⬜ NEXT STEPS (in order):
  ...
  13. ✅ DONE 2026-09-04: **SESSION 4 "STOREFRONT" — LICENSE + README truth pass**:
      (a) LICENSE: MIT, ambrishraj06, 2026 — created (frontmatter said MIT all along, file now matches).
      (b) README overhauled, EVERY claim verified before writing: removed stale HF Spaces YAML frontmatter (repo is deployed on Streamlit Cloud, not HF); hosting claims corrected everywhere (was "HF Spaces 2vCPU/16GB" — now Streamlit Community Cloud 1GB, judge-only by default, HHEM where deps fit); documented the 69-topic built-in library as its own section (named sources WHO/CDC/NICE/ICMR/NHS/AHA, keyword-qualification rules incl. 4x phrase/2x word/umbrella-2-hits, abstain-by-design for unknown diseases, 0-collision gate — ran the gate live: "✅ No keyword collisions"); pipeline diagram now includes [0] match_source auto-pick + [3] holistic_check (conditional, UI explains skip) + threaded live stage events; key-decisions list gained prompt-injection defense, free-tier resilience (backoff/fallback/JSON-defense/memoization), transformers<5 pin note with the HHEM-5.x breakage reason; roadmap item 2 hardening marked DONE.
      (c) NUMBERS RE-VERIFIED against artifacts before quoting (the honest-audit step): stored JSONs carry pre-refinement pass flags (19/20, 23/25) — re-scored the STORED verdicts against CURRENT script labels: battery 20/20 zero mismatches, eval 25/25 zero mismatches, danger 14/14 all BLOCKED, false-SAFE 0 — README numbers are honest under current labels. Also re-ran matcher spot-checks: my first 6 "failures" were wrong EXPECTATIONS on my side (bronchiolitis correctly abstains — not in library; sore-throat→antibiotics-viruses is the legitimate keyword owner whose guideline text covers "most sore throats" — defensible).
      DEPLOY RISK: zero (docs-only, app.py untouched). Health check post-push: VERIFIED ok. README on GitHub renders clean (frontmatter gone, LICENSE linked).
  14. ✅ DONE 2026-09-04: **SESSION 6 "THE STUDENT" — Bot-vs-Bot classroom demo** (builds on the roadmap's Phase-2 item, scoped to the demo loop):
      (a) ENGINE: prompts.py gained PROMPT 5 NAIVE_SYSTEM/NAIVE_USER — a typical un-grounded AI health chatbot persona (confident, specific, plain-text, injection-delimited; NOT told to hallucinate — honest demo). audit.py: generate_naive_answer() added; generate_answer() refactored into shared _generate_plain_text() (same proven retry/backoff/reasoning-effort path, zero duplication). Normal audits completely unchanged.
      (b) REAL-RUN EVIDENCE (all numbers from actual API runs, the demo cases were chosen from these): naive bot on warfarin+thrush → confidently details nystatin/clotrimazole/fluconazole dosing the NICE page never states → BLOCKED 0% (2C/8U/1S, 11 claims) while RAG bot → SAFE 100%; dengue painkiller → naive is GLOBALLY RIGHT (paracetamol, avoid NSAIDs) but invents doses/warning-signs → WARNING 40% (the "true but unverified" lesson); vitamin C → naive WARNING 0% (10/10 unsupported) vs RAG honest refusal.
      (c) UI: "⚔️ Classroom demo" expander — curated selectbox (3 tested cases) + custom question input (needs a library topic), one-click _run_bot_battle (naive gen → audit → RAG gen → audit, 6-8 free-tier calls), textbook-page card, two-column 👦/👧 side-by-side with expanders showing each answer + verdict cards, lesson banner ("unverified is not verified, even when it happens to be true"), history entries labeled "👦 naive bot · q" / "👧 RAG bot · q".
      (d) EDGE FOUND AND FIXED HONESTLY: when the RAG bot refuses (source doesn't cover the question), the judge legitimately flags the refusal sentence as CONTRADICTION ("the source DOES talk about this") → BLOCKED, which would look like the honest bot 'failed'. Demo layer now detects the exact abstention sentence ("does not cover this question") and renders "🙅 RAG BOT DECLINED TO ANSWER — the honest move" card explaining the nuance. Engine verdict untouched — the honesty fix is presentational.
      (e) BUG DURING BUILD: NameError CLASSROOM_CASES (defined after use at module level) → moved definitions above the UI section.
      VERIFIED: compile, 27/27 pytest, AppTest real-API dengue case (both columns + verdict cards + history 2 entries) and vitamin C abstention case (abstention card + lesson info + naive verdict).
      PRODUCTION VERIFIED 2026-09-04 (browser, real runs): warfarin flagship case — textbook card "Warfarin and drug interactions — NICE", 👦 naive column rendered its confident nystatin/clotrimazole/fluconazole dosing → 🚫 AUDIT FAILED (contradiction in 2 of 10 claims); 👧 RAG column answered from the page (fluconazole+warfarin bleeding warning) → ✅ AUDIT PASSED, all 2 claims supported; lesson banner + history both logged; ZERO error cards.
      ❗ DEPLOY INCIDENT + LESSON (2026-09-04, session 6): after push, the Streamlit Cloud container served NEW app.py + STALE audit.py → ImportError generate_naive_answer at app.py:541 (redacted card). Diagnosis chain that PROVED it: top import (line 33) succeeded (page rendered inputs) so medguard.audit imported but lacked the new symbol; raw-GitHub blob hash of audit.py matched local exactly; fresh-interpreter import locally passed; no __pycache__ tracked in git. REMEDY LADDER: (1) empty-commit redeploy — did NOT clear it this time; (2) REAL content change to the stale files (docstring updates that were genuinely due: "Four prompts"→five, generators note) — THAT worked, app loaded clean within ~4 min. LESSON: when a Streamlit Cloud container serves a stale file, an empty commit may not be enough — push an actual content change to the stale path (make it a genuine docs fix, not noise).
...
- ❗ DEPLOY LESSONS (2026-09-04, session 6): (g) a Streamlit Cloud container can serve NEW app.py + STALE engine files after a push (ImportError on the new symbol while the repo blob hash matches local exactly). Remedy ladder: empty-commit redeploy FIRST; if that doesn't clear it within a full rebuild cycle, push a REAL content change to the stale file (make it a genuine docs fix, not noise) — that worked when the empty commit didn't. (h) always smoke-test BOTH import points after a deploy: the top diagnostic-trap import AND any mid-file imports you add later (the trap only covers the top one).
- 15. REMAINING SESSIONS: Session 5 "ENSEMBLE LIVE" (optional — HHEM ONNX ~250MB on Streamlit Cloud; purely a visible second opinion, changes ZERO verdict accuracy — the judge is already 25/25, 14/14 danger recall, 0 false-SAFE). The project is COMPLETE and resume-ready without it.
- 16. ✅ DONE 2026-09-06: **UI REVAMP "OPTION A" — clinical flat-light theme + Light/Dark selector** (user: "no emojis, no AI slop, human-made premium, no gradients, no hover blur, clean and professional for recruiters"):
  - **Theme**: jmedia65 healthcare palette (config.toml: #0066cc/#fafbfc/#f4f6f8/#2c3e50/#dee2e6, showWidgetBorder, baseRadius 0.375rem). Fonts SELF-HOSTED via native `[[theme.fontFaces]]` (IBM Plex Sans 400/600/700 + Mono 400, woff2 in static/fonts/, served at `app/static/...` — verified in installed 1.62 source: `_ROUTE_APP_STATIC = "app/static/{path:path}"`). No Google Fonts CDN.
  - **CSS layer rewritten flat**: white surfaces + 1px hairlines + colored status edge rules. Removed ALL gradients, backdrop-filter, glow/pulse keyframes, transform hovers, dual-ring spinner (now one flat ring). Status colors clinical: #c62828 fail / #1a7f37 pass / #b26a00 caution / #546e7a n/a.
  - **Zero emoji** (70+ sites): page_icon → static/favicon.png (flat navy shield, PIL-marshalled via image_to_url — the ONE page_icon form that works identically local+Cloud; plain-string paths returned EMPTY in offline tests), verdict/chip icons → status-colored text + CSS classes (mg-verdict-BLOCKED etc.), all case keys/labels/buttons/warnings de-emojified.
  - **Dark mode** (user add-on): PALETTES dict (Light/Dark) → one `:root` CSS-vars block per mode; sidebar "Theme" radio (Light default, session-persisted). Dark: #0d131a/#111823/#151e29/#e8eef4/#5aa9e6 + brighter status colors. MG_DARK_CHROME_CSS repaints Streamlit's OWN chrome (inputs, combobox frame, expanders, alerts, stCode+stCodeBlock, header, sidebar, listboxes, buttons, radio markers — unchecked housing recolored; checked keeps primary 16px dot + white 6px center).
  - **Gates built**: scripts/ui_gate.py — static emoji scan + AppTest boot/labels + theme Dark/Light palette assertions + real-API audit (golden → BLOCKED) + real classroom battle. 24/24 PASS. pytest 27/27. Browser computed-style scans (local + production, BOTH modes): 0 gradients, 0 blur, 0 light-surfaces-in-dark, 0 unreadable sidebar text (43/44 nodes dark text color, 1 muted), 0 emoji in DOM, fonts loaded, favicon + wordmark load. Mobile breakpoint: duo 1 column, no overflow.
  - **Deployed + PRODUCTION VERIFIED 2026-09-06** (commits bed3ca5 + 629005f): both modes toggled live on medguardauidt.streamlit.app, full scans clean, restored Light default.
  - ⚠️ Production quirk (NOT a bug in our app): the live app renders INSIDE an `iframe[title="streamlitApp"]` (same-origin) and `stSidebar` only mounts after the iframe session hydrates (~10-15 s). When verifying production: navigate → wait for `.mg-logo` INSIDE `iframe.contentDocument` → then query. A naive host-document query shows "no sidebar / no radios" and is a false alarm.
